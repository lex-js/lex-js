module.exports = class ContentBrowser {
  ...
  navigate(where) {
    ...
    const setActive = el => {
      ...
      this.scrollIfNeeded(
        el,
        document.querySelector("#content-list-container")
      );
    };
  ...
  load(list) {
    ...
    const addEntry = sth => {
      ...
      if (sth.type === "directory") {
        ...
        row.onclick = async () => {
          var url =
            config.content_real_path + state.content_list.path + "/" + filename;

          await files.loadRemote(url, state.content_list.path + "/" + filename);
        ...
        row.onclick = () => {
          state.content_list.path = state.content_list.path
            .split("/")
            .slice(0, -1)
            .join("/");
          this.show();
