...
var DrawControl = {
  ...
  underlineChar: function(char, font, x, y, context) {
    ...
    for (var i = 0; i < imgData1.data.length; i += 4) {
      for (var j = 0; j < 4; j++) {
        // обход r, g, b, a
        if (
          imgData1.data[i + j] == config.fg_color[j] ||
  ...
  redrawSelection: function(context) {
    if (lex.selection.set) {
      ...
      context.fillRect(
        (lex.selection.x1 - lex.screen.x) * config.font_width,
        ...
        for (var j = 0; j < tr.length; j++) {
          ...
          if (r.line >= lex.screen.y && r.line <= lex.screen.y + lex.screen.h) {
            var t =
              config[
                "search_" +
                  (i == lex.search.active_entry_number ? "active_" : "") +
                  "fill_color"
              ];
            context.fillStyle =
              "rgba(" + t[0] + "," + t[1] + "," + t[2] + "," + t[3] / 255 + ")";
            context.fillRect(
              (r.start - lex.screen.x) * config.font_width,
              (r.line - lex.screen.y) * config.font_height,
              r.length * config.font_width,
              config.font_height
            );
          }
        }
...
var LineNumbersControl = {
  ...
  addLineNumbers: function() {
    ...
    for (var i = 0; i < lex.file.lines.length; i++) {
      lex.file.lines[i] = new Uint8Array(
        Parser.getLineNumberBytes(i, lex.numbers.width).concat(
...
var ScreenControl = {
  ...
  checkScrollPosition: function() {
    ...
    var maxshift =
      lex.screen.h - Math.floor(lex.screen.h * config.max_overscroll);
...
var TouchControl = {
  ...
  handleMove: function(event) {
    ...
    for (var i = 0; i < touches.length; i++) {
      ...
      if (idx >= 0) {
        TouchControl.ongoingTouches.splice(
          idx,
          1,
          TouchControl.copyTouch(touches[i])
        );
      } else {
      ...
      if (
        TouchControl.scrollBuffer.y > config.show_top_bar_delta ||
        lex.screen.y === 0
      ) {
        MobileUIControl.showMenu();
      }
      if (
        TouchControl.scrollBuffer.y * -1 > config.hide_top_bar_delta &&
        lex.screen.y !== 0
      ) {
      ...
      if (Math.abs(TouchControl.scrollBuffer.y) > config.touch_y_min) {
        ScreenControl.scrollY(
          (TouchControl.scrollBuffer.y * config.touch_y_speed) /
            config.touch_y_min
        );
      ...
      if (Math.abs(TouchControl.scrollBuffer.x) > config.touch_x_min) {
        ScreenControl.scrollX(
          (TouchControl.scrollBuffer.x * config.touch_x_speed) /
            config.touch_x_min
        );
