...
var Content = {
  show: function() {
    ...
    this.update(this.load, function(err) {
      if (err instanceof Error) {
        ...
        alert(
          "[Can't parse response JSON] You should set up a webserver with PHP support to use content listing."
        );
      }
    });
  ...
  load: function(list) {
    ...
    function addEntry(sth) {
      ...
      icon_img.src =
        config.icon_path +
        { directory: "folder", parent: "up", file: "file" }[sth.type] +
        ".png";
      ...
      if (["directory", "file"].includes(sth.type)) {
        ...
        if (!lex.is_mobile) {
          ...
          mod_td.textContent = new Date(sth.modified)
            .toISOString()
            .slice(0, 16)
            .replace(/T/, " ");
      ...
      if (sth.type === "directory") {
        ...
        tr.onclick = (filename =>
          function() {
            var url =
              config.content_real_path + lex.content_list.path + "/" + filename;
            FileControl.loadFileByURL(
              url,
              lex.content_list.path + "/" + filename,
              function() {
        ...
        tr.onclick = function() {
          lex.content_list.path = lex.content_list.path
            .split("/")
            .slice(0, -1)
            .join("/");
          Content.show();
...
var URIHashControl = {
  ...
  process: function() {
    ...
    if (typeof parsed != "undefined") {
      ...
      if (parsed.type == "remote" && lex.file.remote_name != parsed.file) {
        FileControl.loadFileByURL(
          config.content_real_path + "/" + parsed.file,
          parsed.file,
          () => {
            ...
            // -1 to strip the last `/`
            lex.content_list.path = parsed.file.substr(
              0,
...
var FileControl = {
  ...
  saveFile: function(filename, source) {
    localforage.setItem(config.ls_file_prefix + filename, source, function(
      err,
      value
    ) {
      if (err) {
  ...
  deleteFile: function(filename) {
    localforage.removeItem(
      config.ls_file_prefix + filename,
      GUIControl.updateFileList
    );
  },
    ...
    reader.onload = (function() {
      return function(event) {
        FileControl.saveFile(
          file.name,
          Coders.Uint8ArrayToString(event.target.result)
        );
      };
    })(file);
...
var ExportControl = {
  ...
  exportToPNG: function() {
    ...
    var s = lex.selection;
    var canvas = ExportControl.makeCanvas(
      Math.min(s.x1, s.x2),
      Math.max(s.x1, s.x2),
      Math.min(s.y1, s.y2),
      Math.max(s.y1, s.y2)
    ...
    if (config.export_clear_selection) SelectionControl.clearSelection();
  },
  ...
  exportFileName: function(ext) {
    return (
      config.export_png_file_name_prefix +
      (lex.file.name ? lex.file.name : "")
        .trim()
        .replace(/[^a-z0-9а-яА-Я]/gi, "_")
        .replace(/_{2,}/gi, "_") +
      config.export_png_file_name_suffix +
      (ext ? ext : ext)
    );
  }
};
...
var InitControl = {
  ...
  eventsInit: function() {
    ...
    document
      ...
      .addEventListener("keyup", SearchControl.performSearch);
    document
      ...
      .addEventListener("change", function(evt) {
        ...
        for (var i = 0, f; (f = files[i]); i++) {
          if (!f.type.match("text.*")) {
            ...
            if (["lhs", "txt", "hs", "pas"].indexOf(ext) == -1) continue;
          }
          var reader = new FileReader();
    ...
    document
      .getElementById("button-line-numbers")
    ...
    document
      .getElementById("button-goto-line")
      .addEventListener("click", () => {
        Content.hide();
    ...
    document
      .getElementById("button-content")
      .addEventListener("click", Content.toggle);
    document
      .getElementById("button-open-copy")
      .addEventListener("click", GUIControl.duplicateWindow);
  },
    ...
    var t = {
      ...
      "ctrl+up": function() {
        ScreenControl.scrollY(1 * config.ctrl_scroll_k) ||
          Content.navigate("top");
      },
      "ctrl+down": function() {
        ScreenControl.scrollY(-1 * config.ctrl_scroll_k) ||
          Content.navigate("bottom");
      },
      "ctrl+left": function() {
  ...
  canvasInit: function() {
    ...
    canvas.addEventListener("mousedown", function(event) {
      ...
      lex.selection.x1 =
        lex.screen.x + Math.round(selStartRealX / config.font_width);
      lex.selection.y1 =
        lex.screen.y + Math.round(selStartRealY / config.font_height);
    ...
    function canvasMouseMove(event) {
      if (lex.selection.start) {
        ...
        lex.selection.y2 =
          lex.screen.y + Math.round(selStartRealY / config.font_height);
        if (
          lex.selection.x2 != lex.selection.x1 &&
...
var SearchControl = {
  searchFunctions: [
    ...
    function(text, str) {
      var c,
        ...
        lines = text.split("\n"),
        tn = 0,
        sx = lex.numbers.width
          ? lex.numbers.width + config.line_numbers_padding
          : 0;
  ...
  updateSearchBlock: function() {
    var number =
      lex.search.results.length == 0 ? 0 : lex.search.active_entry_number + 1;
  ...
  scrollToCurrentResult: function() {
    ...
    if (line < lex.screen.y || line > lex.screen.y + lex.screen.h / 2) {
