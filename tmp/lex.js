...
function log(m) {
    // redefine it
    if (config.logging) {
        if (m.length > config.log_max_length) m = m.substr(0, config.log_max_length);
        console.log(m);
    }
}
...
var Parser = {
    ...
    getLineNumberBytes: function getLineNumberBytes(n, width) {
        var r = '                    ';
        var arr = [];
        n = n + '';
        // padding
    ...
    switchCommand: function switchCommand(char, underline, font) {
        // изменить состояние
...
var Coders = {
    // encoders+decoders
    Uint8ArrayToString: function Uint8ArrayToString(ui8arr) {
    ...
    num2Char: function num2Char(num) {
        return String.fromCharCode(num);
    },
    char2Num: function char2Num(chr) {
        return chr.charCodeAt(0);
    },
    dec2Bin: function dec2Bin(dec) {
        return Number(dec).toString(2);
    },
    bin2Dec: function bin2Dec(bin) {
        return parseInt(bin, 2);
    },
    binArray2String: function binArray2String(arr) {
    ...
    string2BinArray: function string2BinArray(str) {
        var len = config.max_char_code;
...
var DrawControl = {
    setFontBGColor: function setFontBGColor(bg_color) {
        config.bg_color = bg_color;
        DrawControl.redrawAll();
    },
    setFontFGColor: function setFontFGColor(fg_color) {
        config.fg_color = fg_color;
        DrawControl.redrawAll();
    },
    makeImageData: function makeImageData() {
    ...
    underlineChar: function underlineChar(char, font, x, y, context) {
    ...
    redrawCanvas: function redrawCanvas(context) {
        var w = lex.screen.w,
            h = lex.screen.h,
        ...
        for (var y = 0; y < h && y < l; y++) {
            ...
            for (var x = 0; x < line.length; x++) {
                ...
                if (lex.fonts[font].bitmaps[char]) {
                    context.putImageData(lex.fonts[font].bitmaps[char], (x - sx) * config.font_width, y * config.font_height);
                    if (underline) {
    ...
    redrawSelection: function redrawSelection(context) {
    ...
    redrawSearchResults: function redrawSearchResults(context) {
    ...
    rebuildIndex: function rebuildIndex() {
        ...
        for (var i = 0; i < lex.file.lines.length; i++) {
            var parsed = Parser.parseLine(lex.file.lines[i]),
                line = Coders.Uint8ArrayToString(parsed.map(function (c) {
                return c.char;
            }));
            lex.index.text += line + '\n';
...
var FontControl = {
    fontsLoaded: function fontsLoaded() {
        for (var fontNumber = 0; fontNumber <= config.font_max; fontNumber++) {
            if (!lex.fonts[fontNumber].source) return false;
        }
        return true;
    },
    loadFont: function loadFont(num, callback) {
        var req = null;
        ...
        req.onreadystatechange = function () {
            if (req.readyState == 4) {
                if (req.status == 200) {
                    ...
                    log('Font loaded: ' + num);
                    if (!!callback) callback(lex.fonts[num]);
                }
            }
        };
        req.send(null);
...
var DevControl = {
    outputFontsToJSFile: function outputFontsToJSFile() {
        ...
        for (var fontNumber = 0; fontNumber <= config.font_max; fontNumber++) {
            ...
            js_output += 'lex.fonts[' + fontNumber + '].source = Coders.string2BinArray(' + uneval(Coders.binArray2String(lex.fonts[fontNumber].source)) + ');\n';
        }
        this.donwloadFile('fonts.js', js_output);
    },
    outputFileToJSFile: function outputFileToJSFile() {
...
var LineNumbersControl = {
    removeLineNumbers: function removeLineNumbers() {
        for (var i = 0; i < lex.file.lines.length; i++) {
    ...
    addLineNumbers: function addLineNumbers() {
    ...
    toggleLineNumbers: function toggleLineNumbers() {
        if (lex.numbers.set) {
...
var SelectionControl = {
    ...
    selectAll: function selectAll() {
        lex.selection.set = 1;
        lex.selection.x1 = 0;
        lex.selection.x2 = Math.max.apply(Math, lex.file.lines.map(function (a) {
            return a.length;
        }));
        lex.selection.y1 = 0;
...
var ScreenControl = {
    ...
    expandScreen: function expandScreen() {
    ...
    scrollHomeY: function scrollHomeY() {
        if (lex.content_tree.active) return;
        lex.screen.y = 0;
        DrawControl.redrawAll();
    },
    scrollEndY: function scrollEndY() {
        if (lex.content_tree.active) return;
        lex.screen.y = lex.file.lines.length - lex.screen.h;
        ScreenControl.checkScrollPosition();
        DrawControl.redrawAll();
    },
    scrollX: function scrollX(x) {
        if (lex.content_tree.active) return;
        x = Math.round(x);
        lex.screen.x -= x;
        ScreenControl.checkScrollPosition();
...
var TouchControl = {
    ...
    copyTouch: function copyTouch(touch) {
        return {
